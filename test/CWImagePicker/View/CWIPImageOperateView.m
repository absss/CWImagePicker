...
- (void)setSelectedCount:(NSInteger)selectedCount{
    ...
    if (selectedCount > 0) {
        NSString * title = [NSString stringWithFormat:@"发送(%ld)",self.selectedCount];
...
- (UIButton *)scanButton{
    if (!_scanButton) {
        ...
        [_scanButton setTitle:@"预览" forState:UIControlStateNormal];
...
- (UIButton *)completeButton{
    if (!_completeButton) {
        ...
        [_completeButton setTitle:@"发送" forState:UIControlStateNormal];
