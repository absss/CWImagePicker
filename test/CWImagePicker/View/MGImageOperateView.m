...
- (void)setSelectedCount:(NSInteger)selectedCount{
    ...
    if (selectedCount > 0) {
        NSString * title = [NSString stringWithFormat:MGLocalString(@"CWIPStr_SendImage"),self.selectedCount];
...
- (UIButton *)scanButton{
    if (!_scanButton) {
        ...
        [_scanButton setTitle:MGLocalString(@"CWIPStr_Scan") forState:UIControlStateNormal];
...
- (UIButton *)completeButton{
    if (!_completeButton) {
        ...
        [_completeButton setTitle:MGLocalString(@"CWIPStr_Send") forState:UIControlStateNormal];
...
- (void)setShowCount:(BOOL)showCount{
    ...
    if (!showCount) {
        ...
        selectedImg = [UIImage imageNamed:@"CWIPResource.bundle/photo_sel_photoPickerVc"];
    }else{
        [self addSubview:self.countLabel];
        selectedImg = [UIImage imageNamed:@"CWIPResource.bundle/photo_number_icon"];
    }
